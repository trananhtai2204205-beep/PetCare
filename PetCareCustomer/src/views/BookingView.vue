...
import { useBookingsStore, BOOKING_TYPES } from '../stores/bookings'
...
const AVAILABLE_DOCTORS = computed(() => bookStore.availableDoctors)

onMounted(async () => {
  await Promise.all([
    petsStore.fetchPets(),
    bookStore.fetchBookings(),
    bookStore.fetchDoctors(),
  ])
})
...
const selectedPetName = computed(() => pets.value.find(p => p.id === bookForm.value.petId)?.name || '')
const selectedDoctorName = computed(() => AVAILABLE_DOCTORS.value.find(d => d.id === bookForm.value.doctorId)?.name || 'Bất kỳ')
