...
function hasRecord(id: string) {
  return recordStore.records.some(r => r.appointmentId === id)
}

async function selectAppointment(appt: Appointment) {
  selectedApptId.value = appt.id
  selectedAppt.value = appt
  const existing = await recordStore.getRecordByAppointment(appt.id)
  if (existing) {
    form.value = {
...
async function saveRecord() {
  ...
  const doctorId = apptStore.appointments.find(a => a.id === selectedAppt.value!.id)?.doctorId || ''
...
async function saveAndComplete() {
  if (!selectedAppt.value) return
  await saveRecord()
  await apptStore.updateStatus(selectedAppt.value.id, 'completed')
